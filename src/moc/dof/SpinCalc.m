...
if i_type~=4 && o_type~=4,  %if input/output are NOT Euler angles
    ...
    if i_type==4,   %if input type is Euler angles, determine the order of rotations
        ...
        %verify angles are between 0 and 360 and identify singularities
        if INPUT(:,1)<zeros(size(INPUT,1),1) | INPUT(:,1)>360*ones(size(INPUT,1),1),
            error('Error: First input Euler angle(s) outside 0 to 360 degree range')
        end
        ...
        else    %Type 1 rotation (all rotationas about each of three axes)
        ...
        if INPUT(:,3)<zeros(size(INPUT,1),1) | INPUT(:,3)>360*ones(size(INPUT,1),1),
            error('Error: Third input Euler angle(s) outside 0 to 360 degree range')
        end        
    end
